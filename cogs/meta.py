...
class Meta(commands.Cog):
    ...
    async def flag(self, ctx, flag=''):
        ...
        if full in flagset:
            ...
            await ctx.send(f'Error: You must use this command with the name of a pride flag. Run **{self.prefix}help flag** for more info.')
        else:
            await ctx.send('Error: The flag you entered was not recognized.')

    @commands.command()
    async def about(self, ctx):
        em = discord.Embed(title=f'About hugmaker ({config["BOT_VERSION"]})', description='Hi, I\'m hugmaker! I am a Discord bot which creates custom emotes.')
        em.add_field(name='Source code', value='Hugmaker is open source under the MIT license! You can find the code at <https://github.com/amazansky/hugmaker>. In addition, any images generated by hugmaker are available under CC BY 4.0. Have any issues or run into problems using the bot? Report them at the issue tracker: <https://github.com/amazansky/hugmaker/issues>', inline=False)
        opstring = ", ".join(["**" + str(await self.bot.fetch_user(uid)) + "**" for uid in self.ops])
        em.add_field(name='Operators', value=f'On this server, the operators of hugmaker are: {opstring}. You can ask them if you have any questions about the bot.', inline=False)
        await ctx.send(embed=em)
